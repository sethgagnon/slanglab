...
-- Admins can update sources, but prevent changing is_required from true to false
...
WITH CHECK (
  ...
  AND (
    ...
    (OLD.is_required = NEW.is_required) OR
    -- Allow changing from false to true
    (OLD.is_required = false AND NEW.is_required = true) OR
    -- Prevent changing from true to false (only super admin could do this)
    (OLD.is_required = true AND NEW.is_required = false AND false)
  )
);
