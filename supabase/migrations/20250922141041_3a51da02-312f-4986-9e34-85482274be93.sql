...
DROP TRIGGER IF EXISTS update_source_rules_updated_at ON public.source_rules;
...
-- Insert default API source configurations with 'allow' status (which exists in the table)
INSERT INTO public.source_rules (source_name, source_type, domain, status, enabled, config) VALUES
('Google Custom Search', 'search_api', 'googleapis.com', 'allow', true, '{"description": "Google Custom Search Engine for web results"}'),
('NewsAPI', 'news_api', 'newsapi.org', 'allow', true, '{"description": "News aggregation service"}'),
('Reddit API', 'social_api', 'reddit.com', 'allow', false, '{"description": "Reddit content search (Coming Soon)"}'),
('YouTube API', 'video_api', 'youtube.googleapis.com', 'allow', false, '{"description": "YouTube video search (Coming Soon)"}')
